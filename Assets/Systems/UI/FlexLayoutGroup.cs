...
/// bounds
/// </summary>
[AddComponentMenu("Layout/Extensions/Flex Layout Group")]
public class FlexLayoutGroup : LayoutGroup
{
    ...
    public float SetLayout(float width, int axis, bool layoutInput)
    {
        ...
        var groupHeight = rect.height;

        // Width that is available after padding is subtracted
        var workingWidth = rect.width - padding.left - padding.right;
        ...
        for (int i = 0, count = rectChildren.Count; i < count; i++)
        {
            ...
            var index = IsLowerAlign ? count - 1 - i : i;
    ...
    protected void LayoutRow(IList<RectTransform> contents, float rowWidth, float rowHeight, float maxWidth, float xOffset, float yOffset, int axis)
    {
        ...
        int rowListCount = _rowList.Count;
        ...
        for (var j = 0; j < rowListCount; j++)
        {
            var index = IsLowerAlign ? rowListCount - 1 - j : j;
